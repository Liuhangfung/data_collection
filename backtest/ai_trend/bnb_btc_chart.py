#!/usr/bin/env python3
"""
BNB/BTC All-Time Chart Generator
Creates comprehensive charts showing the BNB/BTC ratio over time with yearly ATH/ATL points
"""
...
class BNBBTCChartGenerator:
    """Generate all-time BNB/BTC ratio charts"""
    ...
    def calculate_bnb_btc_ratio(self, bnb_data, btc_data):
        """Calculate BNB/BTC ratio from price data"""
        print("🔄 Calculating BNB/BTC ratio...")
        
        # Align timestamps and calculate ratio
        combined = pd.merge(
            bnb_data[['close']].rename(columns={'close': 'bnb_price'}),
        ...
        combined['bnb_btc_ratio'] = combined['bnb_price'] / combined['btc_price']
        
        print(f"✅ Calculated ratio for {len(combined)} data points")
        print(f"   Current BNB/BTC ratio: {combined['bnb_btc_ratio'].iloc[-1]:.6f}")
        print(f"   All-time high: {combined['bnb_btc_ratio'].max():.6f} on {combined['bnb_btc_ratio'].idxmax().strftime('%Y-%m-%d')}")
        print(f"   All-time low: {combined['bnb_btc_ratio'].min():.6f} on {combined['bnb_btc_ratio'].idxmin().strftime('%Y-%m-%d')}")
        
        return combined
    
    def create_bnb_btc_chart(self, ratio_data, save_path=None):
        """Create comprehensive BNB/BTC ratio chart"""
        print("📈 Creating BNB/BTC ratio chart...")
        ...
        # Chart 1: BNB/BTC Ratio Over Time
        ax1.plot(ratio_data.index, ratio_data['bnb_btc_ratio'], 
                color='#F3BA2F', linewidth=1.5, alpha=0.8, label='BNB/BTC Ratio')  # Binance yellow/gold
        
        # Find yearly ATH and ATL points
        ratio_data['year'] = ratio_data.index.year
        yearly_ath = ratio_data.groupby('year')['bnb_btc_ratio'].idxmax()
        yearly_atl = ratio_data.groupby('year')['bnb_btc_ratio'].idxmin()
        ...
        ax1.scatter(yearly_ath_data.index, yearly_ath_data['bnb_btc_ratio'], 
                   color='green', s=120, marker='^', alpha=0.9, 
                   label='Yearly ATH', zorder=5, edgecolors='darkgreen', linewidth=1)
        
        # Plot yearly ATL points  
        ax1.scatter(yearly_atl_data.index, yearly_atl_data['bnb_btc_ratio'], 
                   color='red', s=120, marker='v', alpha=0.9, 
                   label='Yearly ATL', zorder=5, edgecolors='darkred', linewidth=1)
        
        # Highlight overall ATH and ATL
        ath_date = ratio_data['bnb_btc_ratio'].idxmax()
        ath_value = ratio_data['bnb_btc_ratio'].max()
        atl_date = ratio_data['bnb_btc_ratio'].idxmin()
        atl_value = ratio_data['bnb_btc_ratio'].min()
        ...
        ax1.set_title('BNB/BTC Ratio with Yearly ATH/ATL Points', fontsize=18, fontweight='bold', pad=20)
        ax1.set_ylabel('BNB/BTC Ratio', fontsize=16)
        ...
        # BNB price (left axis)
        line1 = ax2.plot(ratio_data.index, ratio_data['bnb_price'], 
                        color='#F3BA2F', linewidth=2, label='BNB Price')
        ax2.set_ylabel('BNB Price (USDT)', fontsize=14, color='#F3BA2F')
        ax2.tick_params(axis='y', labelcolor='#F3BA2F')
        ...
        ax2.set_title('BNB vs BTC Price Comparison', fontsize=16, fontweight='bold', pad=20)
        ...
        current_bnb = ratio_data['bnb_price'].iloc[-1]
        current_btc = ratio_data['btc_price'].iloc[-1]
        current_ratio = ratio_data['bnb_btc_ratio'].iloc[-1]
        
        # Calculate changes
        ratio_change_30d = ((current_ratio / ratio_data['bnb_btc_ratio'].iloc[-30]) - 1) * 100
        ratio_change_1y = ((current_ratio / ratio_data['bnb_btc_ratio'].iloc[-365]) - 1) * 100 if len(ratio_data) > 365 else 0
        
        # Create info boxes on the chart
        # Current prices info box (top right)
        current_info = f"""BNB: ${current_bnb:.2f}
BTC: ${current_btc:,.0f}
BNB/BTC: {current_ratio:.6f}"""
        ...
        for i, (date, row) in enumerate(yearly_ath_data.iterrows()):
            year = date.year
            info_text = f"{year} ATH\nBNB: ${row['bnb_price']:.2f}\nBTC: ${row['btc_price']:.0f}\nBNB/BTC: {row['bnb_btc_ratio']:.6f}"
            ...
            ax1.annotate(info_text, xy=(date, row['bnb_btc_ratio']), 
...
Date Range: {ratio_data.index[0].strftime('%Y-%m-%d')} to {ratio_data.index[-1].strftime('%Y-%m-%d')}"""
    ...
    def generate_comprehensive_analysis(self, timeframe='1d', years_back=8):
        """Generate complete BNB/BTC analysis with charts"""
        print("=" * 60)
        print("🚀 BNB/BTC ALL-TIME CHART GENERATOR")
        print("=" * 60)
        
        # Fetch BNB data
        bnb_data = self.fetch_historical_data('BNB/USDT', timeframe, years_back)
        if bnb_data is None:
            print("❌ Failed to fetch BNB data")
            return None
        ...
        # Calculate ratio
        ratio_data = self.calculate_bnb_btc_ratio(bnb_data, btc_data)
        
        # Create charts
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = f"bnb_btc_chart_{timestamp}.png"
        
        self.create_bnb_btc_chart(ratio_data, save_path)
        
        print("\n" + "=" * 60)
        print("✅ BNB/BTC Analysis Complete!")
...
def main():
    """Main function to generate BNB/BTC chart"""
    try:
        # Create chart generator
        chart_gen = BNBBTCChartGenerator()
        
        # Generate analysis
        # BNB has been around since 2017, so 8 years covers its history
        ratio_data = chart_gen.generate_comprehensive_analysis(
            timeframe='1d',
            years_back=8  # BNB launched in 2017
        )
        
        if ratio_data is not None:
            print("\n📈 Additional Analysis Available:")
            print("   - BNB/BTC ratio trends")
