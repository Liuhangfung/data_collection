#!/usr/bin/env python3
"""
ETH/BTC All-Time Chart Generator
Creates comprehensive charts showing the ETH/BTC ratio over time with various timeframes
"""
...
class ETHBTCChartGenerator:
    """Generate all-time ETH/BTC ratio charts"""
    ...
    def fetch_historical_data(self, symbol, timeframe='1d', years_back=8):
        """
    ...
    def calculate_eth_btc_ratio(self, eth_data, btc_data):
        """Calculate ETH/BTC ratio from price data"""
        print("🔄 Calculating ETH/BTC ratio...")
        
        # Align timestamps and calculate ratio
        combined = pd.merge(
            eth_data[['close']].rename(columns={'close': 'eth_price'}),
        ...
        combined['eth_btc_ratio'] = combined['eth_price'] / combined['btc_price']
        
        print(f"✅ Calculated ratio for {len(combined)} data points")
        print(f"   Current ETH/BTC ratio: {combined['eth_btc_ratio'].iloc[-1]:.6f}")
        print(f"   All-time high: {combined['eth_btc_ratio'].max():.6f} on {combined['eth_btc_ratio'].idxmax().strftime('%Y-%m-%d')}")
        print(f"   All-time low: {combined['eth_btc_ratio'].min():.6f} on {combined['eth_btc_ratio'].idxmin().strftime('%Y-%m-%d')}")
        
        return combined
    
    def create_eth_btc_chart(self, ratio_data, save_path=None):
        """Create comprehensive ETH/BTC ratio chart"""
        print("📈 Creating ETH/BTC ratio chart...")
        ...
        fig.suptitle('ETH/BTC All-Time Chart Analysis', fontsize=24, fontweight='bold', y=0.95)
        
        # Chart 1: ETH/BTC Ratio Over Time
        ax1.plot(ratio_data.index, ratio_data['eth_btc_ratio'], 
                color='#F7931A', linewidth=1.5, alpha=0.8, label='ETH/BTC Ratio')
        
        # Find yearly ATH and ATL points
        ratio_data['year'] = ratio_data.index.year
        yearly_ath = ratio_data.groupby('year')['eth_btc_ratio'].idxmax()
        yearly_atl = ratio_data.groupby('year')['eth_btc_ratio'].idxmin()
        ...
        ax1.scatter(yearly_ath_data.index, yearly_ath_data['eth_btc_ratio'], 
                   color='green', s=120, marker='^', alpha=0.9, 
                   label='Yearly ATH', zorder=5, edgecolors='darkgreen', linewidth=1)
        
        # Plot yearly ATL points  
        ax1.scatter(yearly_atl_data.index, yearly_atl_data['eth_btc_ratio'], 
                   color='red', s=120, marker='v', alpha=0.9, 
                   label='Yearly ATL', zorder=5, edgecolors='darkred', linewidth=1)
        
        # Highlight overall ATH and ATL
        ath_date = ratio_data['eth_btc_ratio'].idxmax()
        ath_value = ratio_data['eth_btc_ratio'].max()
        atl_date = ratio_data['eth_btc_ratio'].idxmin()
        atl_value = ratio_data['eth_btc_ratio'].min()
        ...
        ax1.set_title('ETH/BTC Ratio with Yearly ATH/ATL Points', fontsize=18, fontweight='bold', pad=20)
        ax1.set_ylabel('ETH/BTC Ratio', fontsize=16)
        ...
        # ETH price (left axis)
        line1 = ax2.plot(ratio_data.index, ratio_data['eth_price'], 
                        color='#627EEA', linewidth=2, label='ETH Price')
        ax2.set_ylabel('ETH Price (USDT)', fontsize=14, color='#627EEA')
        ax2.tick_params(axis='y', labelcolor='#627EEA')
        ...
        ax2.set_title('ETH vs BTC Price Comparison', fontsize=16, fontweight='bold', pad=20)
        ...
        current_eth = ratio_data['eth_price'].iloc[-1]
        current_btc = ratio_data['btc_price'].iloc[-1]
        current_ratio = ratio_data['eth_btc_ratio'].iloc[-1]
        
        # Calculate changes
        ratio_change_30d = ((current_ratio / ratio_data['eth_btc_ratio'].iloc[-30]) - 1) * 100
        ratio_change_1y = ((current_ratio / ratio_data['eth_btc_ratio'].iloc[-365]) - 1) * 100 if len(ratio_data) > 365 else 0
        
        # Create info boxes on the chart
        # Current prices info box (top right)
        current_info = f"""ETH: ${current_eth:,.0f}
BTC: ${current_btc:,.0f}
ETH/BTC: {current_ratio:.6f}"""
        ...
        for i, (date, row) in enumerate(yearly_ath_data.iterrows()):
            year = date.year
            info_text = f"{year} ATH\nETH: ${row['eth_price']:.0f}\nBTC: ${row['btc_price']:.0f}\nETH/BTC: {row['eth_btc_ratio']:.6f}"
            ...
            ax1.annotate(info_text, xy=(date, row['eth_btc_ratio']), 
...
Date Range: {ratio_data.index[0].strftime('%Y-%m-%d')} to {ratio_data.index[-1].strftime('%Y-%m-%d')}"""
    ...
    def generate_comprehensive_analysis(self, timeframe='1d', years_back=8):
        """Generate complete ETH/BTC analysis with charts"""
        print("=" * 60)
        print("🚀 ETH/BTC ALL-TIME CHART GENERATOR")
        print("=" * 60)
        
        # Fetch ETH data
        eth_data = self.fetch_historical_data('ETH/USDT', timeframe, years_back)
        if eth_data is None:
        ...
        # Calculate ratio
        ratio_data = self.calculate_eth_btc_ratio(eth_data, btc_data)
        
        # Create charts
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = f"eth_btc_chart_{timestamp}.png"
        
        self.create_eth_btc_chart(ratio_data, save_path)
        
        print("\n" + "=" * 60)
        print("✅ ETH/BTC Analysis Complete!")
...
def main():
    """Main function to generate ETH/BTC chart"""
    try:
        # Create chart generator
        chart_gen = ETHBTCChartGenerator()
        
        # Generate analysis
        # You can modify timeframe ('1d', '1w') and years_back as needed
        ratio_data = chart_gen.generate_comprehensive_analysis(
            timeframe='1d',
            years_back=8  # Fetch 8 years of data for comprehensive view
        )
        
        if ratio_data is not None:
            print("\n📈 Additional Analysis Available:")
            print("   - ETH/BTC ratio trends")
            print("   - Moving average analysis") 
            print("   - All-time high/low identification")
