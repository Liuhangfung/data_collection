#!/usr/bin/env python3
"""
SOL/BTC All-Time Chart Generator
Creates comprehensive charts showing the SOL/BTC ratio over time with yearly ATH/ATL points
"""
...
class SOLBTCChartGenerator:
    """Generate all-time SOL/BTC ratio charts"""
    ...
    def fetch_historical_data(self, symbol, timeframe='1d', years_back=6):
        """
    ...
    def calculate_sol_btc_ratio(self, sol_data, btc_data):
        """Calculate SOL/BTC ratio from price data"""
        print("🔄 Calculating SOL/BTC ratio...")
        
        # Align timestamps and calculate ratio
        combined = pd.merge(
            sol_data[['close']].rename(columns={'close': 'sol_price'}),
        ...
        combined['sol_btc_ratio'] = combined['sol_price'] / combined['btc_price']
        
        print(f"✅ Calculated ratio for {len(combined)} data points")
        print(f"   Current SOL/BTC ratio: {combined['sol_btc_ratio'].iloc[-1]:.6f}")
        print(f"   All-time high: {combined['sol_btc_ratio'].max():.6f} on {combined['sol_btc_ratio'].idxmax().strftime('%Y-%m-%d')}")
        print(f"   All-time low: {combined['sol_btc_ratio'].min():.6f} on {combined['sol_btc_ratio'].idxmin().strftime('%Y-%m-%d')}")
        
        return combined
    
    def create_sol_btc_chart(self, ratio_data, save_path=None):
        """Create comprehensive SOL/BTC ratio chart"""
        print("📈 Creating SOL/BTC ratio chart...")
        ...
        fig.suptitle('SOL/BTC All-Time Chart Analysis', fontsize=24, fontweight='bold', y=0.95)
        
        # Chart 1: SOL/BTC Ratio Over Time
        ax1.plot(ratio_data.index, ratio_data['sol_btc_ratio'], 
                color='#9945FF', linewidth=1.5, alpha=0.8, label='SOL/BTC Ratio')
        
        # Find yearly ATH and ATL points
        ratio_data['year'] = ratio_data.index.year
        yearly_ath = ratio_data.groupby('year')['sol_btc_ratio'].idxmax()
        yearly_atl = ratio_data.groupby('year')['sol_btc_ratio'].idxmin()
        ...
        ax1.scatter(yearly_ath_data.index, yearly_ath_data['sol_btc_ratio'], 
                   color='green', s=120, marker='^', alpha=0.9, 
                   label='Yearly ATH', zorder=5, edgecolors='darkgreen', linewidth=1)
        
        # Plot yearly ATL points  
        ax1.scatter(yearly_atl_data.index, yearly_atl_data['sol_btc_ratio'], 
                   color='red', s=120, marker='v', alpha=0.9, 
                   label='Yearly ATL', zorder=5, edgecolors='darkred', linewidth=1)
        
        # Highlight overall ATH and ATL
        ath_date = ratio_data['sol_btc_ratio'].idxmax()
        ath_value = ratio_data['sol_btc_ratio'].max()
        atl_date = ratio_data['sol_btc_ratio'].idxmin()
        atl_value = ratio_data['sol_btc_ratio'].min()
        
        ax1.scatter(ath_date, ath_value, color='darkgreen', s=300, marker='^', 
                   label=f'Overall ATH: {ath_value:.6f}', zorder=6, edgecolors='white', linewidth=2)
        ax1.scatter(atl_date, atl_value, color='darkred', s=300, marker='v', 
                   label=f'Overall ATL: {atl_value:.6f}', zorder=6, edgecolors='white', linewidth=2)
        
        ax1.set_title('SOL/BTC Ratio with Yearly ATH/ATL Points', fontsize=18, fontweight='bold', pad=20)
        ax1.set_ylabel('SOL/BTC Ratio', fontsize=16)
        ...
        # SOL price (left axis)
        line1 = ax2.plot(ratio_data.index, ratio_data['sol_price'], 
                        color='#9945FF', linewidth=2, label='SOL Price')
        ax2.set_ylabel('SOL Price (USDT)', fontsize=14, color='#9945FF')
        ax2.tick_params(axis='y', labelcolor='#9945FF')
        ...
        ax2.set_title('SOL vs BTC Price Comparison', fontsize=16, fontweight='bold', pad=20)
        ...
        current_sol = ratio_data['sol_price'].iloc[-1]
        current_btc = ratio_data['btc_price'].iloc[-1]
        current_ratio = ratio_data['sol_btc_ratio'].iloc[-1]
        
        # Calculate changes
        ratio_change_30d = ((current_ratio / ratio_data['sol_btc_ratio'].iloc[-30]) - 1) * 100
        ratio_change_1y = ((current_ratio / ratio_data['sol_btc_ratio'].iloc[-365]) - 1) * 100 if len(ratio_data) > 365 else 0
        
        # Create info boxes on the chart
        # Current prices info box (top right)
        current_info = f"""SOL: ${current_sol:,.2f}
BTC: ${current_btc:,.0f}
SOL/BTC: {current_ratio:.6f}"""
        ...
        for i, (date, row) in enumerate(yearly_ath_data.iterrows()):
            year = date.year
            info_text = f"{year} ATH\nSOL: ${row['sol_price']:.2f}\nBTC: ${row['btc_price']:.0f}\nSOL/BTC: {row['sol_btc_ratio']:.6f}"
            ...
            ax1.annotate(info_text, xy=(date, row['sol_btc_ratio']), 
...
Date Range: {ratio_data.index[0].strftime('%Y-%m-%d')} to {ratio_data.index[-1].strftime('%Y-%m-%d')}"""
    ...
    def generate_comprehensive_analysis(self, timeframe='1d', years_back=6):
        """Generate complete SOL/BTC analysis with charts"""
        print("=" * 60)
        print("🚀 SOL/BTC ALL-TIME CHART GENERATOR")
        print("=" * 60)
        
        # Fetch SOL data
        sol_data = self.fetch_historical_data('SOL/USDT', timeframe, years_back)
        if sol_data is None:
        ...
        # Calculate ratio
        ratio_data = self.calculate_sol_btc_ratio(sol_data, btc_data)
        
        # Create charts
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = f"sol_btc_chart_{timestamp}.png"
        
        self.create_sol_btc_chart(ratio_data, save_path)
        
        print("\n" + "=" * 60)
        print("✅ SOL/BTC Analysis Complete!")
...
def main():
    """Main function to generate SOL/BTC chart"""
    try:
        # Create chart generator
        chart_gen = SOLBTCChartGenerator()
        
        # Generate analysis
        # SOL launched around 2020, so 6 years should cover most of its history
        ratio_data = chart_gen.generate_comprehensive_analysis(
            timeframe='1d',
            years_back=6  # SOL is newer than ETH, so fewer years available
        )
        
        if ratio_data is not None:
            print("\n📈 Additional Analysis Available:")
            print("   - SOL/BTC ratio trends")
