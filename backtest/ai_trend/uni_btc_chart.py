#!/usr/bin/env python3
"""
UNI/BTC All-Time Chart Generator
Creates comprehensive charts showing the UNI/BTC ratio over time with yearly ATH/ATL points
"""
...
class UNIBTCChartGenerator:
    """Generate all-time UNI/BTC ratio charts"""
    ...
    def fetch_historical_data(self, symbol, timeframe='1d', years_back=5):
        """
        Fetch historical data for a given symbol
        
        Args:
            symbol: Trading pair (e.g., 'UNI/USDT', 'BTC/USDT')
    ...
    def calculate_uni_btc_ratio(self, uni_data, btc_data):
        """Calculate UNI/BTC ratio from price data"""
        print("🔄 Calculating UNI/BTC ratio...")
        
        # Align timestamps and calculate ratio
        combined = pd.merge(
            uni_data[['close']].rename(columns={'close': 'uni_price'}),
        ...
        combined['uni_btc_ratio'] = combined['uni_price'] / combined['btc_price']
        
        print(f"✅ Calculated ratio for {len(combined)} data points")
        print(f"   Current UNI/BTC ratio: {combined['uni_btc_ratio'].iloc[-1]:.8f}")
        print(f"   All-time high: {combined['uni_btc_ratio'].max():.8f} on {combined['uni_btc_ratio'].idxmax().strftime('%Y-%m-%d')}")
        print(f"   All-time low: {combined['uni_btc_ratio'].min():.8f} on {combined['uni_btc_ratio'].idxmin().strftime('%Y-%m-%d')}")
        
        return combined
    
    def create_uni_btc_chart(self, ratio_data, save_path=None):
        """Create comprehensive UNI/BTC ratio chart"""
        print("📈 Creating UNI/BTC ratio chart...")
        ...
        fig.suptitle('UNI/BTC All-Time Chart Analysis', fontsize=24, fontweight='bold', y=0.95)
        
        # Chart 1: UNI/BTC Ratio Over Time
        ax1.plot(ratio_data.index, ratio_data['uni_btc_ratio'], 
                color='#FF007A', linewidth=1.5, alpha=0.8, label='UNI/BTC Ratio')  # Uniswap pink/magenta
        
        # Find yearly ATH and ATL points
        ratio_data['year'] = ratio_data.index.year
        yearly_ath = ratio_data.groupby('year')['uni_btc_ratio'].idxmax()
        yearly_atl = ratio_data.groupby('year')['uni_btc_ratio'].idxmin()
        ...
        ax1.scatter(yearly_ath_data.index, yearly_ath_data['uni_btc_ratio'], 
                   color='green', s=120, marker='^', alpha=0.9, 
                   label='Yearly ATH', zorder=5, edgecolors='darkgreen', linewidth=1)
        
        # Plot yearly ATL points  
        ax1.scatter(yearly_atl_data.index, yearly_atl_data['uni_btc_ratio'], 
                   color='red', s=120, marker='v', alpha=0.9, 
                   label='Yearly ATL', zorder=5, edgecolors='darkred', linewidth=1)
        
        # Highlight overall ATH and ATL
        ath_date = ratio_data['uni_btc_ratio'].idxmax()
        ath_value = ratio_data['uni_btc_ratio'].max()
        atl_date = ratio_data['uni_btc_ratio'].idxmin()
        atl_value = ratio_data['uni_btc_ratio'].min()
        ...
        ax1.set_title('UNI/BTC Ratio with Yearly ATH/ATL Points', fontsize=18, fontweight='bold', pad=20)
        ax1.set_ylabel('UNI/BTC Ratio', fontsize=16)
        ...
        # UNI price (left axis)
        line1 = ax2.plot(ratio_data.index, ratio_data['uni_price'], 
                        color='#FF007A', linewidth=2, label='UNI Price')
        ax2.set_ylabel('UNI Price (USDT)', fontsize=14, color='#FF007A')
        ax2.tick_params(axis='y', labelcolor='#FF007A')
        ...
        ax2.set_title('UNI vs BTC Price Comparison', fontsize=16, fontweight='bold', pad=20)
        ...
        current_uni = ratio_data['uni_price'].iloc[-1]
        current_btc = ratio_data['btc_price'].iloc[-1]
        current_ratio = ratio_data['uni_btc_ratio'].iloc[-1]
        
        # Calculate changes
        ratio_change_30d = ((current_ratio / ratio_data['uni_btc_ratio'].iloc[-30]) - 1) * 100
        ratio_change_1y = ((current_ratio / ratio_data['uni_btc_ratio'].iloc[-365]) - 1) * 100 if len(ratio_data) > 365 else 0
        
        # Create info boxes on the chart
        # Current prices info box (top right)
        current_info = f"""UNI: ${current_uni:.3f}
BTC: ${current_btc:,.0f}
UNI/BTC: {current_ratio:.8f}"""
        ...
        for i, (date, row) in enumerate(yearly_ath_data.iterrows()):
            year = date.year
            info_text = f"{year} ATH\nUNI: ${row['uni_price']:.3f}\nBTC: ${row['btc_price']:.0f}\nUNI/BTC: {row['uni_btc_ratio']:.8f}"
            ...
            ax1.annotate(info_text, xy=(date, row['uni_btc_ratio']), 
...
Date Range: {ratio_data.index[0].strftime('%Y-%m-%d')} to {ratio_data.index[-1].strftime('%Y-%m-%d')}"""
    ...
    def generate_comprehensive_analysis(self, timeframe='1d', years_back=5):
        """Generate complete UNI/BTC analysis with charts"""
        print("=" * 60)
        print("🚀 UNI/BTC ALL-TIME CHART GENERATOR")
        print("=" * 60)
        
        # Fetch UNI data
        uni_data = self.fetch_historical_data('UNI/USDT', timeframe, years_back)
        if uni_data is None:
            print("❌ Failed to fetch UNI data")
            return None
        ...
        # Calculate ratio
        ratio_data = self.calculate_uni_btc_ratio(uni_data, btc_data)
        
        # Create charts
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = f"uni_btc_chart_{timestamp}.png"
        
        self.create_uni_btc_chart(ratio_data, save_path)
        
        print("\n" + "=" * 60)
        print("✅ UNI/BTC Analysis Complete!")
...
def main():
    """Main function to generate UNI/BTC chart"""
    try:
        # Create chart generator
        chart_gen = UNIBTCChartGenerator()
        
        # Generate analysis
        # UNI launched in 2020, so 5 years covers its entire history
        ratio_data = chart_gen.generate_comprehensive_analysis(
            timeframe='1d',
            years_back=5  # UNI launched in September 2020
        )
        
        if ratio_data is not None:
            print("\n📈 Additional Analysis Available:")
            print("   - UNI/BTC ratio trends")
