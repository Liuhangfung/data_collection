#!/usr/bin/env python3
"""
XRP/BTC All-Time Chart Generator
Creates comprehensive charts showing the XRP/BTC ratio over time with yearly ATH/ATL points
"""
...
class XRPBTCChartGenerator:
    """Generate all-time XRP/BTC ratio charts"""
    ...
    def fetch_historical_data(self, symbol, timeframe='1d', years_back=10):
        """
        Fetch historical data for a given symbol
        
        Args:
            symbol: Trading pair (e.g., 'XRP/USDT', 'BTC/USDT')
    ...
    def calculate_xrp_btc_ratio(self, xrp_data, btc_data):
        """Calculate XRP/BTC ratio from price data"""
        print("🔄 Calculating XRP/BTC ratio...")
        
        # Align timestamps and calculate ratio
        combined = pd.merge(
            xrp_data[['close']].rename(columns={'close': 'xrp_price'}),
        ...
        combined['xrp_btc_ratio'] = combined['xrp_price'] / combined['btc_price']
        
        print(f"✅ Calculated ratio for {len(combined)} data points")
        print(f"   Current XRP/BTC ratio: {combined['xrp_btc_ratio'].iloc[-1]:.8f}")
        print(f"   All-time high: {combined['xrp_btc_ratio'].max():.8f} on {combined['xrp_btc_ratio'].idxmax().strftime('%Y-%m-%d')}")
        print(f"   All-time low: {combined['xrp_btc_ratio'].min():.8f} on {combined['xrp_btc_ratio'].idxmin().strftime('%Y-%m-%d')}")
        
        return combined
    
    def create_xrp_btc_chart(self, ratio_data, save_path=None):
        """Create comprehensive XRP/BTC ratio chart"""
        print("📈 Creating XRP/BTC ratio chart...")
        ...
        # Chart 1: XRP/BTC Ratio Over Time
        ax1.plot(ratio_data.index, ratio_data['xrp_btc_ratio'], 
                color='#23292F', linewidth=1.5, alpha=0.8, label='XRP/BTC Ratio')  # XRP dark blue/black
        
        # Find yearly ATH and ATL points
        ratio_data['year'] = ratio_data.index.year
        yearly_ath = ratio_data.groupby('year')['xrp_btc_ratio'].idxmax()
        yearly_atl = ratio_data.groupby('year')['xrp_btc_ratio'].idxmin()
        ...
        ax1.scatter(yearly_ath_data.index, yearly_ath_data['xrp_btc_ratio'], 
                   color='green', s=120, marker='^', alpha=0.9, 
                   label='Yearly ATH', zorder=5, edgecolors='darkgreen', linewidth=1)
        
        # Plot yearly ATL points  
        ax1.scatter(yearly_atl_data.index, yearly_atl_data['xrp_btc_ratio'], 
                   color='red', s=120, marker='v', alpha=0.9, 
                   label='Yearly ATL', zorder=5, edgecolors='darkred', linewidth=1)
        
        # Highlight overall ATH and ATL
        ath_date = ratio_data['xrp_btc_ratio'].idxmax()
        ath_value = ratio_data['xrp_btc_ratio'].max()
        atl_date = ratio_data['xrp_btc_ratio'].idxmin()
        atl_value = ratio_data['xrp_btc_ratio'].min()
        ...
        ax1.set_title('XRP/BTC Ratio with Yearly ATH/ATL Points', fontsize=18, fontweight='bold', pad=20)
        ax1.set_ylabel('XRP/BTC Ratio', fontsize=16)
        ...
        # XRP price (left axis)
        line1 = ax2.plot(ratio_data.index, ratio_data['xrp_price'], 
                        color='#23292F', linewidth=2, label='XRP Price')
        ax2.set_ylabel('XRP Price (USDT)', fontsize=14, color='#23292F')
        ax2.tick_params(axis='y', labelcolor='#23292F')
        ...
        ax2.set_title('XRP vs BTC Price Comparison', fontsize=16, fontweight='bold', pad=20)
        ...
        current_xrp = ratio_data['xrp_price'].iloc[-1]
        current_btc = ratio_data['btc_price'].iloc[-1]
        current_ratio = ratio_data['xrp_btc_ratio'].iloc[-1]
        
        # Calculate changes
        ratio_change_30d = ((current_ratio / ratio_data['xrp_btc_ratio'].iloc[-30]) - 1) * 100
        ratio_change_1y = ((current_ratio / ratio_data['xrp_btc_ratio'].iloc[-365]) - 1) * 100 if len(ratio_data) > 365 else 0
        
        # Create info boxes on the chart
        # Current prices info box (top right)
        current_info = f"""XRP: ${current_xrp:.4f}
BTC: ${current_btc:,.0f}
XRP/BTC: {current_ratio:.8f}"""
        ...
        for i, (date, row) in enumerate(yearly_ath_data.iterrows()):
            ...
            ax1.annotate(info_text, xy=(date, row['xrp_btc_ratio']), 
        ...
        for i, (date, row) in enumerate(yearly_atl_data.iterrows()):
            ...
            if i % 2 == 0:  # 1st, 3rd, 5th... go LEFT
                offset_x = -200
                offset_y = -50 - (i * 12)  # Spread vertically downward
                connection_style = 'arc3,rad=0.3'
            else:  # 2nd, 4th, 6th... go RIGHT
                offset_x = 200
                offset_y = -50 - (i * 12)  # Spread vertically downward
...
Date Range: {ratio_data.index[0].strftime('%Y-%m-%d')} to {ratio_data.index[-1].strftime('%Y-%m-%d')}"""
    ...
    def generate_comprehensive_analysis(self, timeframe='1d', years_back=10):
        """Generate complete XRP/BTC analysis with charts"""
        print("=" * 60)
        print("🚀 XRP/BTC ALL-TIME CHART GENERATOR")
        print("=" * 60)
        
        # Fetch XRP data
        xrp_data = self.fetch_historical_data('XRP/USDT', timeframe, years_back)
        if xrp_data is None:
            print("❌ Failed to fetch XRP data")
            return None
        ...
        # Calculate ratio
        ratio_data = self.calculate_xrp_btc_ratio(xrp_data, btc_data)
        
        # Create charts
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = f"xrp_btc_chart_{timestamp}.png"
        
        self.create_xrp_btc_chart(ratio_data, save_path)
        
        print("\n" + "=" * 60)
        print("✅ XRP/BTC Analysis Complete!")
...
def main():
    """Main function to generate XRP/BTC chart"""
    try:
        # Create chart generator
        chart_gen = XRPBTCChartGenerator()
        
        # Generate analysis
        # XRP has been around since 2012, so 10 years covers much of its history
        ratio_data = chart_gen.generate_comprehensive_analysis(
            timeframe='1d',
            years_back=10  # XRP is one of the oldest cryptocurrencies
        )
        
        if ratio_data is not None:
            print("\n📈 Additional Analysis Available:")
            print("   - XRP/BTC ratio trends")
